...
当前主线已增加智能检索、知识图谱、阅读模式辅助阅读、用户画像、推荐、概念词典和管理员任务观测接口；论文解析由 `POST /api/papers/{paper_id}/parse` 调度当前 FastAPI 进程内的后台任务完成，未配置模型时使用本地降级摘要。
...
| `GET` | `/api/papers/{paper_id}` | 读取数据库论文 |
| `GET` | `/api/papers/{paper_id}/wiki` | 读取数据库结构化结果；未生成时回退论文摘要 |
| `POST` | `/api/papers/{paper_id}/qa` | 数据库论文的可追溯问答 |
