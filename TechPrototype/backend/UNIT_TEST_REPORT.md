...
后端智能检索内核的语句覆盖率为 **94.14%**，高于 90% 的验收要求；覆盖率命令启用了 `--cov-fail-under=90`，低于阈值时会返回非零退出码并使任务失败。
...
| 模块 | 语句数 | 未覆盖 | 语句覆盖率 |
|---|---:|---:|---:|
| `search_query_normalize.py` | 208 | 15 | 92.79% |
...
- 年份区间、`after/since`、`before/until`、精确年份和“近几年”。
- 综述/短文排除词和论文元数据排除判断。
