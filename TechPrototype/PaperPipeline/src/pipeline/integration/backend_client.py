...
class BackendClient:
    ...
    def update_task(self, task_id: int, status: str, error_code: str | None = None, stage: str | None = None, lease_token: str | None = None) -> dict:
        body: dict[str, Any] = {"status": status}
        if error_code:
            body["error_code"] = error_code[:64]
        if stage:
            body["stage"] = stage[:32]
        headers = {"X-Task-Lease": lease_token} if lease_token else None
        return self._request(f"/api/tasks/{task_id}", "PATCH", body, headers)
    ...
    def save_structured_results(self, task_id: int, results: list[dict[str, Any]], lease_token: str | None = None) -> dict:
        headers = {"X-Task-Lease": lease_token} if lease_token else None
        return self._request(f"/api/tasks/{task_id}/results", "POST", {"results": results}, headers)
    ...
    def finalize_parse_result(self, task_id: int, chunks: list[dict[str, Any]], results: list[dict[str, Any]], lease_token: str | None = None) -> dict:
        headers = {"X-Task-Lease": lease_token} if lease_token else None
        return self._request(
