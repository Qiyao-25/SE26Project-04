...
def action_create(payload: UserActionInput, request: Request, db: Session = Depends(db_session)):
    try:
...
def action_update(action_id: int, payload: UserActionUpdate, request: Request, db: Session = Depends(db_session)):
    try:
        data = update_action(db, action_id, payload)
    except ValueError as exc:
        return map_error(request, exc)
    return ApiResponse(data=data, request_id=request.state.request_id)
...
def action_delete(action_id: int, request: Request, db: Session = Depends(db_session)):
    try:
