...
async function askMockPaper({ conversationId, paperId, question, history = [] }) {
  ...
  if (paperId === qaMock.data.paperId) return { ...qaMock.data, conversationId: conversationId || qaMock.data.conversationId, messageId: `assistant-${Date.now()}`, createdAt: new Date().toISOString() };
  const paper = PAPERS[paperId];
  if (!paper) throw new Error('论文不存在');
  return { conversationId: conversationId || `conversation-${paperId}-${Date.now()}`, messageId: `assistant-${Date.now()}`, paperId, answer: `关于《${paper.title}》：${paper.summary} 你可以继续追问论文的方法、实验、创新点或局限性。`, createdAt: new Date().toISOString(), citations: [{ citationId: `citation-${paperId}-${Date.now()}`, paperId, paperTitle: paper.title, sectionId: 'mock-summary', sectionTitle: '结构化摘要', pageNumber: 1, quote: paper.summary }], historyCount: history.length };
}

export async function askPaper(payload) {
  if (USE_MOCK) return askMockPaper(payload);
  return apiClient.post(`/papers/${payload.paperId}/qa`, payload);
}
