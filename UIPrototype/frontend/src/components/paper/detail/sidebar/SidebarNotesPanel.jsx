...
export default function SidebarNotesPanel({ paperId }) {
  ...
  const reload = async () => {
    ...
    setLoading(true);
    try {
      const [privateActions, publicComments] = await Promise.all([
  ...
  const handleSaveNote = async () => {
    ...
    if (noteMode === 'annotation' && !quote.trim()) {
      return message.warning('批注请先在 PDF 正文划选，或手改摘录');
    }
    const highlightRects = quoteMeta.rects?.length ? quoteMeta.rects : undefined;
    const payload = {
      kind: noteMode === 'annotation' ? 'annotation' : 'note',
      text,
      highlight: quote.trim() || undefined,
      page_no: quoteMeta.pageNo ?? undefined,
