...
$FinalRelease = Split-Path -Parent $SourceRoot
$RepoRoot = Split-Path -Parent $FinalRelease
...
if (-not $Pkg) {
  throw "No papermate-*.tar.gz found. Run: python FinalRelease/source/deploy/pack.py  (or set PACKAGES_DIR)"
}
...
[void]$SeedCandidates.Add((Join-Path $SourceRoot "PaperPipeline\data\seed.json"))
...
Write-Host "See FinalRelease/docs/others/服务器运维与更新.md"
