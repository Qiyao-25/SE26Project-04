...
Usage (from repo root):
  ...
  python FinalRelease/source/deploy/pack.py --out dist/packages
  # 中间产物建议输出到 ppp 工作区：
  python FinalRelease/source/deploy/pack.py --out ../ppp/deploy-artifacts/packages
...
def build_frontend(stage: Path) -> None:
    ...
    example = SOURCE_ROOT / "deploy" / "env.frontend.production.example"
...
def stage_backend(stage: Path) -> None:
    ...
    shutil.copy2(SOURCE_ROOT / "deploy" / "env.backend.production.example", target / ".env.production.example")
...
def main() -> int:
    ...
    if not args.skip_frontend_build:
        ...
        dist = FRONTEND / "dist"
        if not dist.exists():
            raise SystemExit("--skip-frontend-build requires existing FinalRelease/source/frontend/dist")
        shutil.copytree(dist, stage / "frontend")

    stage_backend(stage)
    shutil.copytree(SOURCE_ROOT / "deploy", stage / "deploy", dirs_exist_ok=True)
    ...
    for name in ("nginx.papermate.conf.example", "papermate-backend.service.example",
        ...
        shutil.copy2(SOURCE_ROOT / "deploy" / name, deploy_bundle / name)
    docs = REPO_ROOT / "FinalRelease" / "docs" / "others" / "部署说明.md"
    if docs.exists():
