...
4. 启动顺序：先在 `backend` 启动 Uvicorn（可配置 `PAPERMATE_LLM_API_KEY`），再在 `FinalRelease/source/frontend` 执行 `npm run dev`。登录、注册、账户修改和学习数据会通过后端保存；点击论文详情页的解析按钮后，由 FastAPI 当前进程直接执行解析，不需要另开 Worker。
