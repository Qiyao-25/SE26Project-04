...
默认使用 `FinalRelease/source/backend/data/dev.db`，PDF 缓存位于 `data/pdfs/`。这些目录会随首次启动自动创建，仓库只保留目录占位文件，不提交数据库、PDF 和运行时配置。相对路径也会自动锚定到后端目录，因此从仓库根目录启动不会把数据写到其他位置。测试使用内存 SQLite，不会污染开发数据库。PostgreSQL 只需替换：
